...
Der bestehende freiwillige Campstandort bleibt die einzige Standortquelle. Neben dem bisherigen Kartenlink im `app_settings`-Datensatz `camp_location_link` liegen dort optional die normalisierte Bezeichnung, Breiten-/Laengengrad und Zeitzone. Beim administrativen Speichern loest die Edge Function `geocode_camp_location` die sichtbare Ortsangabe ueber die Open-Meteo-Geocoding-API auf. Erst ein vollstaendiges internes Ergebnis wird zusammen mit dem weiterhin unterstuetzten Kartenlink ueber `ha_update_camp_location_link` gespeichert. Aendern oder Entfernen verwirft Koordinaten, Zeitzone und den Wettercache gemeinsam.
