...
Die Kuenstleruebersicht unter `#artists` verwendet dieselben, bereits mit dem Timetable geladenen Acts. Sie dedupliziert nach Act-ID, sortiert und filtert die Namen clientseitig. `#artists/<act-id>` ist bereits als gueltige Route vorbereitet und zeigt bis zur spaeteren Detailansicht weiterhin die Uebersicht; es gibt keine zweite Kuenstlerdatenquelle.
