-- Recalculate the complete dependent bracket and propagate later-round byes.
...
begin
  ...
  for v_round_index in 0..jsonb_array_length(v_bracket->'rounds') - 1 loop
    ...
    for v_match_index in 0..jsonb_array_length(
      ...
      v_winner_resolution := v_match->>'winnerResolution';
      ...
      if v_match_id = p_match_id then
        ...
        v_winner_resolution := 'manual';
      elsif (v_slot_a_id is null) <> (v_slot_b_id is null) then
        v_winner_id := coalesce(v_slot_a_id, v_slot_b_id);
        v_winner_resolution := 'automatic';
      elsif v_slot_a_id is null and v_slot_b_id is null then
        v_winner_id := null;
        v_winner_resolution := null;
      elsif v_winner_resolution = 'automatic'
      ...
      v_bracket := jsonb_set(
        ...
        array['rounds', v_round_index::text, 'matches', v_match_index::text,
          'winnerResolution'],
        coalesce(to_jsonb(v_winner_resolution), 'null'::jsonb)
      );
      v_bracket := jsonb_set(
        v_bracket,
        array['rounds', v_round_index::text, 'matches', v_match_index::text,
