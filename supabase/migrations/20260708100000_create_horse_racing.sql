-- Add festival-scoped Horse Racing betting with locked-down RPC access.

create table if not exists public.horse_racing_settings (
  festival_id text primary key,
  is_enabled boolean not null default false,
  betting_status text not null default 'closed',
  updated_at timestamptz not null default now(),
  updated_by_participant_id text,
  constraint horse_racing_settings_betting_status_check
    check (betting_status in ('open', 'closed'))
);

create table if not exists public.horse_racing_bets (
  ...
  constraint horse_racing_bets_suit_check
...
revoke all on table public.horse_racing_settings from anon, authenticated;
revoke all on table public.horse_racing_bets from anon, authenticated;

create policy "deny direct horse racing settings access"
  on public.horse_racing_settings
  for all
  using (false)
  with check (false);

create policy "deny direct horse racing bet access"
  on public.horse_racing_bets
  for all
  using (false)
  with check (false);
...
begin
  ...
  return query
  select
    coalesce(hrs.festival_id, p_festival_id) as festival_id,
  ...
  from (select p_festival_id as festival_id) requested_festival
  left join public.horse_racing_settings hrs
    on hrs.festival_id = requested_festival.festival_id
  left join public.horse_racing_bets hrb
    on hrb.festival_id = requested_festival.festival_id
...
begin
  ...
  where p_festival_id is not null
    and btrim(p_festival_id) <> ''
  group by hrs.festival_id, hrs.is_enabled, hrs.betting_status, hrs.updated_at;
end;
$$;
